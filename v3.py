...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        return torch.cat([h(x) for h in self.heads], dim=-1) # concatenate the outputs of all heads along the last dimension
...
class FeedForward(nn.Module):
    ...
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embd, n_embd), # first linear layer expands the dimension
            nn.ReLU()
        )
...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.sa_head(x) # (B,T,C) -> (B,T, vocab_size)
